...
export default function SelectFieldInput(props){
  return <fieldset className={`border rounded ${props.className ? props.className : '' }`} style={props.style}>
    ...
    <select className={`form-select border-0 ${props.classNameInput}`} style={{fontSize:props.fontSize}} defaultValue={props.defaultValue?props.defaultValue:t('no data')} value={props.value} onChange={e =>props.onChange(e.target.value)} disabled={props.disabled}>
      {props.children}
    </select>
  </fieldset>
}