...
export default function TextAreaFieldInput(props){
  return <fieldset className={`border rounded ${props.className ? props.className : '' }`} style={props.style}>
    ...
    <textarea className={`border-0 px-2 py-1 ${props.classNameInput?props.classNameInput:''}`} style={{ width:'100%',height:'100%',outline:'none',fontSize:props.fontSize,resize:"vertical"}} type={props.type} value={props.value} onChange={e=>props.onChange(e.target.value)} onBlur={props.onBlur} disabled={props.disabled}
