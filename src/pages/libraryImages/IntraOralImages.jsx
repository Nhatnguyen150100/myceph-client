...
export default function IntraoralImages(props){
  ...
  const getListImage = () => {
    return new Promise((resolve, reject) => {
      ...
      getToServerWithToken(`/v1/libraryImagePatient/${patient.currentPatient.id}?typeImages=intraoral`).then(result => {
  ...
  const deleteImagePatient = (isDelete) => {
    ...
    return new Promise((resolve,reject) => {
      if(isDelete){
        deleteToServerWithToken(`/v1/libraryImagePatient/${patient.currentPatient.id}?idImage=${idImageDelete}&typeImages=intraloral`).then(result => {
          setListImage(result.data);
          setOpenDeleteConfirm(false);
          setIdImageDelete('');
          setPublicIdDelete('');
                ...
                listImage[date]?.map((image,_) => {
                  return <div onMouseEnter={e=>{if(roleCheck) setHoverSettingId(image.id)}} onMouseLeave={e=>{if(roleCheck) setHoverSettingId()}} className=" position-relative mt-3" key={image.id}>
                    <img 
                      ...
                      alt={t(IMAGE_TYPE_LIST.INTRA_ORAL.name)} 
