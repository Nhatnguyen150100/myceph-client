...
export default function Issue(props){
  ...
  useEffect(()=>{
    if(patient.currentPatient.id) getListOfIssues();
  ...
  const getListOfIssues = () => {
    ...
    return new Promise((resolve, reject) =>{
      getToServerWithToken(`/v1/listOfIssue/${patient.currentPatient.id}?mode=${props.checkRoleMode}&idDoctor=${doctor.data?.id}&page=${page}&pageSize=${PAGE_SIZE}`).then(result => {
  ...
  const updateIssue = () => {
    ...
    return new Promise((resolve, reject) =>{
      ...
      putToServerWithToken(`/v1/listOfIssue/updateIssue/${patient.currentPatient.id}?idIssue=${editIssueId}&mode=${props.checkRoleMode}&idDoctor=${doctor.data?.id}&page=${page}&pageSize=${PAGE_SIZE}`,infoUpdate).then(result => {
        setListOfIssue(isEncrypted?deCryptedListIssue(result.data):result.data);
        setCount(result.count)
        toast.success(t(result.message));
        resolve();
  ...
  return <div className="h-100 w-100 d-flex flex-column justify-content-start mt-1">
          ...
          return <fieldset className="border row rounded mt-3" key={issue.id}>
            ...
            <div className="row">
              <div className="col-sm-4">
                <fieldset className='border-0 rounded me-2 w-100'>
                  <legend style={{ fontSize: '1rem'}} className='w-auto mb-0 ms-2 float-none px-2 text-uppercase fw-bold'>
                    {t('issue')}{' '}{(page-1)*PAGE_SIZE+index+1}
                  </legend>
                  <textarea 
              ...
              <div className="col-sm-4">
                <fieldset className='border-0 rounded me-2 w-100'>
                  <legend style={{ fontSize: '1rem'}} className='w-auto mb-0 ms-2 float-none px-2 text-uppercase fw-bold'>
                    {t('treatment object')}{' '}{(page-1)*PAGE_SIZE+index+1}
                  </legend>
                  <textarea
                    onKeyDown={e=>{if(e.key === "Enter") updateIssue(issue.id) ; if(e.key === "Escape") setEditIssueId('')}} 
              ...
              <div className="col-sm-4">
                <fieldset className='border-0 rounded me-2 w-100'>
                  <legend style={{ fontSize: '1rem'}} className='w-auto mb-0 ms-2 float-none px-2 text-uppercase fw-bold'>
                    {t('treatment method')}{' '}{(page-1)*PAGE_SIZE+index+1}
                  </legend>
                  <textarea
                    onKeyDown={e=>{if(e.key === "Enter") updateIssue(issue.id) ; if(e.key === "Escape") setEditIssueId('')}} 
    ...
    <ConfirmComponent 
      ...
      content={
        <div>
          <span className="me-1" style={{fontSize:FONT_SIZE}}>{t('Do you want to delete this issue?')}</span>
        </div>
      }
