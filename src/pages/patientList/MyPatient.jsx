  ...
  const deletePatient = (idPatient) => {
    if(idPatient===idPatientDelete){
      ...
      return new Promise((resolve, reject) => {
        ...
        deleteToServerWithToken(`/v1/patient/deletePatient/${idPatient}?mode=unCheck&idDoctor=${doctor?.id}`).then(result=>{
    ...
    <table className="table table-bordered table-striped text-center rounded my-4">
      <thead className='mc-background text-white text-uppercase'>
        <tr>
          ...
          <th className='align-middle mc-heading-middle d-lg-table-cell action-col' style={{fontSize:FONT_SIZE}}>{t("action")}</th>
        </tr>
      </thead>
      {
