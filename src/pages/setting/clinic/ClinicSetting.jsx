...
export default function ClinicSetting(props){
  ...
  <div className="w-100 d-flex justify-content-end mc-background-color-white py-2 rounded">
    <div className="d-flex flex-row align-items-center justify-content-between">
      <button type="button" className={`btn me-1 px-3 py-0  text-white-hover ${selectedTab===0?'mc-pale-background text-white':'hoverGreenLight'}`} onClick={e=>dispatch(setClinicSettingTab(0))}>
        <span className="text-capitalize" style={{fontSize:FONT_SIZE}}>{t('my clinic profile')}</span>
      </button>
      {
        clinic.idClinicDefault && <button type="button" className={`btn me-31 px-3 py-0 text-white-hover ${selectedTab===1?'mc-pale-background text-white':'hoverGreenLight'}`} onClick={e=>dispatch(setClinicSettingTab(1))}>
          <span className="text-capitalize" style={{fontSize:FONT_SIZE}}>{t('member of clinic')}</span>
        </button>
      }
      {
        clinic.idClinicDefault && <button type="button" className={`btn me-1 px-3 py-0 text-white-hover ${selectedTab===2?'mc-pale-background text-white':'hoverGreenLight'}`} onClick={e=>dispatch(setClinicSettingTab(2))}>
          <span className="text-capitalize" style={{fontSize:FONT_SIZE}}>{t('rooms of clinic')}</span>
        </button>
      }
      {
        clinic.idClinicDefault && <button type="button" className={`btn me-1 px-3 py-0 text-white-hover ${selectedTab===3?'mc-pale-background text-white':'hoverGreenLight'}`} onClick={e=>dispatch(setClinicSettingTab(3))}>
